...
class Simulation:
    ...
    def simulate_console(self, wait=False, save_to_file=True, json_file_path="alive.csv", csv_file_path="pos.json"):
        ...
        for i in range(self.round_number):
            if len(self.sheep) == 0:
                break
            dead_sheep = self.one_round()
            num_of_dead_sheep = '-'
            if dead_sheep is not None:
                num_of_dead_sheep = self.get_number_of_dead_sheep(dead_sheep)
                logging.info(
    ...
    def get_number_of_dead_sheep(self, dead_sheep):
        return self.starters_sheep.index(dead_sheep) + 1
